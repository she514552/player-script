...
class Workflow {
	...
	private extractTceGlobalVariable(
		jscode: string,
		playerId: string,
	): TceVariable | null {
		const match = jscode.match(this.TCE_GLOBAL_VARIABLE_PATTERN);
		if (
			!match?.groups?.varname ||
			!match?.groups?.code ||
			!match?.groups?.value
		) {
			console.error(
				`Failed to find the tce global variable for player ${playerId} in the jscode.`,
			);
			return null;
		}
	...
	private extractTceNsigFunction(
		jscode: string,
		playerId: string,
	): string | null {
		...
		console.error(
			`Failed to extract tce variant n param tranformation function for player ${playerId} from the jscode.`,
		);
		return null;
	}

	private extractTceSigFunction(
		jscode: string,
		playerId: string,
	): string | null {
		...
		console.error(
			`Failed to extract tce variant s param transformation function for player ${playerId} from the jscode.`,
		);
		return null;
	}

	private extractTceVariantSigFunctionActions(
		jscode: string,
		playerId: string,
	): string | null {
		...
		console.error(
			`Failed to extract tce variant s param transformation function actions for player ${playerId} from the jscode.`,
		);
		return null;
	}
	...
	private extractSignatureTimestamp(
		...
		if (!match?.[2]) {
			console.error(
				`Failed to extract the signature timestamp for player ${playerId} from the jscode.`,
			);
			return null;
		}
	...
	private fixNsigFunctionCode(
		...
		const pattern = new RegExp(
			`;\\s*if\\s*\\(\\s*typeof\\s+[a-zA-Z0-9_$]+\\s*===?\\s*(?:"undefined"|'undefined'|${tce.name.replace("$", "\\$")}\\[\\d+\\])\\s*\\)\\s*return\\s+\\w+;`,
		);
		const match = functionCode.match(pattern);

		if (match) {
			console.warn(
				`Short-Circuit detected in n param tranformation code. Fixing the player ${playerId} n function code...`,
			);
	...
	async run(): Promise<void> {
		...
		const signatureTimestamp = this.extractSignatureTimestamp(
			jsScript,
			playerId,
		);
		...
		if (missing.length > 0) {
			throw new Error(
				`Workflow failed: could not extract ${missing.join(", ")} for player ${playerId}`,
			);
		}
